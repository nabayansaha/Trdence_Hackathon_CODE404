...
import chromadb
from chromadb.config import Settings as ChromaSettings # this for online
...
class RAG:
    def __init__(self, text_or_path, chroma_host='localhost', chroma_port=8000, chroma_ssl=False):
        logger.info("Initializing RAG with Online Chroma")
    ...
    def _get_chroma_client(self):
        """Create a Chroma client for online connection"""
        logger.info(f"Connecting to Chroma at {self.chroma_host}:{self.chroma_port}")
        
        chroma_settings = ChromaSettings(
            chroma_server_host=self.chroma_host,
            chroma_server_http_port=self.chroma_port,
            chroma_server_ssl_enabled=self.chroma_ssl
        )
    ...
    def create_db(self, db_name):
        ...
        logger.info(f"Successfully created online vector index {db_name} with {len(llama_nodes)} nodes")
        return index

    def update_db(self, db_name, new_text):
        logger.info(f"Updating online vector database: {db_name}")
        ...
        logger.info(f"Updating existing online Chroma collection: {db_name}")
    ...
    def create_retriever(self, db_name):
        logger.info(f"Creating retriever for online database: {db_name}")
        ...
        # Otherwise, try to load from the online database
        try:
            # Use online Chroma client
            chroma_client = self._get_chroma_client()
            ...
            logger.info(f"Successfully created retriever from online database: {db_name}")
