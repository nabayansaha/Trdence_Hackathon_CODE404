...
class RAG:
    ...
    def create_db(self, db_name):
        ...
        # Create a unique path for each database
        db_path = os.path.join("chroma_db", db_name)
        if os.path.exists(db_path):
            import shutil

            shutil.rmtree(db_path)

        # Create a new client instance for each database
        chroma_client = chromadb.PersistentClient(path=db_path)
        # Use the db_name as collection name and ensure it's unique
        ...
        logger.info(
            f"Successfully created vector index {db_name} with {len(llama_nodes)} nodes"
        )
        return index
    ...
    def update_db(self, db_name, new_text):
        ...
        # Use the same unique path for the database
        db_path = os.path.join("chroma_db", db_name)
        chroma_client = chromadb.PersistentClient(path=db_path)
        collection_name = f"collection_{db_name}"
        ...
        logger.info(f"Updating existing Chroma collection: {collection_name}")
        ...
        # Use insert_nodes method to add new nodes
    ...
    def rag_query(self, query_text, retriever):
        ...
        sorted_nodes = sorted(
            retrieval_result, key=lambda node: node.metadata.get("chunk_index", 0)
        )
        ...
        llm = Chat()
        updated_messages, input_tokens, output_tokens = llm.invoke_llm_langchain(
            messages=message
        )
        logger.info(
...
if __name__ == "__main__":
    ...
    while True:
        action = (
            input(
                "Enter 'query' to ask a question, 'update' to add new data, or 'q' to quit: "
            )
            .strip()
            .lower()
        )
        ...
        if action == "q":
            ...
            company = (
                input("Enter company name (company_a/company_b): ").strip().lower()
            )
            ...
            update_type = (
                input(
                    "Enter 'file' to update from a file or 'text' to enter raw text: "
                )
                .strip()
                .lower()
            )
            ...
            if update_type == "file":
                new_doc_path = input(
                    "Enter the path of the new document to add: "
                ).strip()
                try:
            ...
            indexes[company] = rag_instances[company].update_db(
                db_name=str(company), new_text=new_text
            )
            retrievers[company] = indexes[company].as_retriever()
